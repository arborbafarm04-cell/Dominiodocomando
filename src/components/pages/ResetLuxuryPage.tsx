...
export default function ResetLuxuryPage() {
  ...
  const handleResetLuxury = async () => {
    ...
    try {
      ...
      console.error('Erro ao resetar itens de luxo:', err);
    } finally {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-black overflow-hidden">
      ...
      <div className="fixed inset-0 z-0">
        ...
        <div className="absolute top-0 left-1/4 w-96 h-96 bg-red-500/10 rounded-full blur-3xl opacity-30" />
        <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-red-600/5 rounded-full blur-3xl opacity-25" />
      </div>
          ...
          >
            {!resetComplete ? (
              <div className="relative">
                ...
                <div className="absolute -inset-4 bg-gradient-to-b from-red-500/20 to-red-600/10 rounded-2xl blur-2xl opacity-50" />
                
                <div className="relative bg-gradient-to-br from-slate-900/95 via-slate-950/95 to-black/95 border-2 border-red-500/50 rounded-2xl overflow-hidden backdrop-blur-xl p-8">
                  {/* Top accent line */}
                  ...
                  >
                    ...
                    <div>
                      <p className="font-paragraph text-sm text-red-300/70">Ação Irreversível</p>
                      <h2 className="font-heading text-3xl text-red-100">Resetar Itens de Luxo</h2>
                    </div>
                  </motion.div>
                  ...
                  >
                    <div className="p-6 bg-red-500/10 border border-red-500/30 rounded-lg">
                      <p className="font-paragraph text-base text-red-200/90 leading-relaxed">
                        Esta ação irá resetar <strong>TODOS</strong> os itens de luxo comprados e restaurar o estado inicial do Showroom de Luxo.
                      </p>
                    </div>

                    {/* Stats */}
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-4 bg-slate-800/50 border border-red-500/20 rounded-lg">
                        <p className="font-paragraph text-xs text-red-200/60 uppercase tracking-wider mb-2">Itens Comprados</p>
                        <p className="font-heading text-2xl text-red-400 font-bold">{purchasedItems.length}</p>
                      </div>
                      <div className="p-4 bg-slate-800/50 border border-red-500/20 rounded-lg">
                        <p className="font-paragraph text-xs text-red-200/60 uppercase tracking-wider mb-2">Dinheiro Sujo</p>
                        <p className="font-heading text-2xl text-red-400 font-bold">R$ {dirtMoney.toLocaleString('pt-BR')}</p>
                      </div>
                    </div>

                    <div className="p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                      <p className="font-paragraph text-sm text-yellow-200/80">
                        ⚠️ Após o reset, você será redirecionado para o Showroom de Luxo. Esta ação não pode ser desfeita.
                      </p>
                    </div>
                  </motion.div>
                  ...
                  {/* Bottom accent line */}
                  <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-red-400 to-transparent" />
                </div>
              </div>
            ) : (
              <motion.div
                ...
                <div className="relative bg-gradient-to-br from-slate-900/95 via-slate-950/95 to-black/95 border-2 border-green-500/50 rounded-2xl overflow-hidden backdrop-blur-xl p-8 text-center">
                  ...
                  >
                    ...
                    <p className="font-paragraph text-green-200/70 mb-6">
                      Todos os itens de luxo foram resetados com sucesso. Você será redirecionado em breve...
                    </p>
